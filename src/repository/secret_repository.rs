...
lazy_static! {
    static ref SECRET_META_CACHE: Cache<String, entity::secret_meta::Model> =
...
pub async fn insert_secret_meta<C: ConnectionTrait>(
    db: &C,
    model: &entity::secret_meta::Model,
) -> Result<()> {
...
pub async fn select_secret<C: ConnectionTrait>(
    db: &C,
    key_id: &str,
) -> Result<Option<entity::secret::Model>> {
    Ok(Secret::find()
        .filter(entity::secret::Column::KeyId.eq(key_id))
        .one(db)
        .await?)
}
...
pub async fn select_secret_meta<C: ConnectionTrait>(
    db: &C,
    key_id: &str,
) -> Result<Option<entity::secret_meta::Model>> {
    Ok(match SECRET_META_CACHE.get(key_id).await {
        Some(secret_meta) => Some(secret_meta),
        None => {
            SecretMeta::find()
                .filter(entity::secret_meta::Column::KeyId.eq(key_id))
                .one(db)
                .await?
        }
    })
